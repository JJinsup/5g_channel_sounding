...
%
% This project copy keeps the captured waveform path of the MathWorks
% example. The local waveform-generation/AWGN tutorial path was removed so
% this file stays focused on real OTA captures. The receiver performs these
% steps:
%
% * _Captured waveform input_: Load an OTA I/Q capture and its receiver
% parameters.
...
% This project keeps only the real-capture path from the MathWorks example.
% The original tutorial also contains a local waveform-generation/AWGN branch
...
if ~exist("minChannelBWOverride","var")
    minChannelBWOverride = 40; % n79 100 MHz deployment; CORESET 0 table selector
end
...
% Symbol phase compensation frequency. Specify the carrier center frequency
% or set to 0 to disable symbol phase compensation.
fPhaseComp = rx.fPhaseComp; % Carrier center frequency (Hz)

% Set the minimum channel bandwidth for the NR band required to configure
% CORESET 0 in FR1 (See TS 38.101-1 Table 5.3.5-1).
if isempty(minChannelBWOverride)
    minChannelBW = rx.minChannelBW;
else
    minChannelBW = minChannelBWOverride;
end

% Configure necessary burst parameters at the receiver. The SSB pattern can
% be 'Case A','Case B','Case C' for FR1 or 'Case D','Case E' for FR2. The
% maximum number of blocks L_max can be 4 or 8 for FR1 and 64 for FR2.
...
rxGrid = rxGrid(:,2:5,:);
ssbGrid = rxGrid; % Preserve the detected SSB grid for CSI export.
...
if sampleRate < nrb*12*scsCommon*1e3
    ...
    fprintf("Minimum sample rate for CORESET 0: %.2f MS/s (%d RB @ %d kHz SCS)\n", ...
        requiredSampleRate/1e6,nrb,scsCommon);
...
% Calculate RMS PDSCH EVM in %, including normalization of PDSCH symbols
% for any offset between DM-RS and PDSCH power
