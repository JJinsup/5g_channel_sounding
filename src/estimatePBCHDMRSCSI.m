...
%ESTIMATEPBCHDMRSCSI Extract PBCH-DMRS-conditioned sparse LS channel samples.
%   The current implementation uses a PBCH-DMRS hypothesis, scans the
%   demodulated local grid with a 240x4 sliding SSB window, and keeps the
%   block with the strongest PBCH-DMRS energy for the selected PCI.
...
pbchResult.hypothesis = struct( ...
    'pci', pci, ...
    'dmrsIbarSsb', cfg.pbch.dmrsIbarSsb, ...
    'bestLocalSymbolStart', bestStartSymbol, ...
    'selectionMethod', 'highest PBCH-DMRS energy local block');
pbchResult.measuredObservables = struct( ...
    'receivedPbchDmrsSamples', rxDmrs, ...
    'hypothesisConditionedSparseLsChannelSamples', lsEstimate, ...
    'hypothesisConditionedSparseLsChannelGrid', sparseCSI, ...
    'measuredSparseRefMask', ~isnan(sparseCSI));
pbchResult.qualityMetrics = struct( ...
    'bestHypothesisBlockMeanEnergy', bestEnergy, ...
    'measuredSparseRefCount', numel(lsEstimate), ...
    'measuredSparseRefRatio', numel(lsEstimate) / numel(bestBlock), ...
    'measuredSparseRefPerSymbol', sum(~isnan(sparseCSI), 1));
end
