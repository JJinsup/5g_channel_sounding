...
- figure 저장을 켜면 공식 예제 figure와 프로젝트에서 추가한 CSI figure가 `outputs/figures/<capture-file-name>/figures.pdf` 하나의 multipage PDF로 저장된다.
...
```text
outputs/figures/<capture-file-name>/figures.pdf
```
...
```text
outputs/processed/<capture-file-name>_mib_sib1_recovery.mat
```
...
```text
outputs/logs/mib_sib1_batch_<timestamp>.mat
```
...
PBCH DM-RS CSI refs: 144
PDSCH DM-RS CSI refs: 612
Figure output: outputs/figures/61.44_260507/figures.pdf
```
